import chessBoard
import chessPieces
from transpositionTables import TranspositionTables
...
class ChessAI:
    ...
    def evaluate(board):
        ...
        pawns = ChessAI.get_piece_transposition_score(board, chessPieces.Pawn.CHESS_PIECE_TYPE, TranspositionTables.PAWN_PIECE_TABLE)
        knights = ChessAI.get_piece_transposition_score(board, chessPieces.Knight.CHESS_PIECE_TYPE, TranspositionTables.KNIGHT_PIECE_TABLE)
        bishops = ChessAI.get_piece_transposition_score(board, chessPieces.Bishop.CHESS_PIECE_TYPE, TranspositionTables.BISHOP_PIECE_TABLE)
        rooks = ChessAI.get_piece_transposition_score(board, chessPieces.Rook.CHESS_PIECE_TYPE, TranspositionTables.ROOK_PIECE_TABLE)
        queens = ChessAI.get_piece_transposition_score(board, chessPieces.Queen.CHESS_PIECE_TYPE, TranspositionTables.QUEEN_PIECE_TABLE)
    ...
    def get_material_score(board):
        ...
        for x_pos in range(8):
            for y_pos in range(8):
                ...
                if piece != 0:
                    if piece.piece_color == chessPieces.ChessPiece.CHESS_PIECE_WHITE:
    ...
    def get_ai_move(chessboard, illegal_moves):
        ...
        for move in chessboard.get_legal_moves(chessPieces.ChessPiece.CHESS_PIECE_BLACK):
            ...
            #alpha beta pruning
            score = ChessAI.alpha_beta_pruning(copy, 2, -ChessAI.INFINITE, ChessAI.INFINITE, True)
        ...
        if (copy.is_check(chessPieces.ChessPiece.CHESS_PIECE_BLACK)):
    ...
    def minimax(board, depth, maximizing):
        ...
        #get legal moves
        for move in board.get_legal_moves(chessPieces.ChessPiece.CHESS_PIECE_WHITE if maximizing else chessPieces.ChessPiece.CHESS_PIECE_BLACK):
            
            #create copy of the board and make the move
            copy = chessBoard.Board.duplicate(board)
            copy.make_move(move)
    ...
    def alpha_beta_pruning(chessboard, depth, a, b, maximizing):
        ...
        #get legal moves
        for move in chessboard.get_legal_moves(chessPieces.ChessPiece.CHESS_PIECE_WHITE if maximizing else chessPieces.ChessPiece.CHESS_PIECE_BLACK):
