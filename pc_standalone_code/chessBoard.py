...
class Board:
        ...
        for y in range(8):
            ...
            for char in row:
                if char.isdigit():
                    ...
                    color = chessPieces.ChessPiece.CHESS_PIECE_WHITE if char.isupper() else chessPieces.ChessPiece.CHESS_PIECE_BLACK
                    piece_type = char.lower()
                    piece = None
                    if piece_type == 'p':
                        piece = chessPieces.Pawn(x, y, color)
                    elif piece_type == 'r':
                        piece = chessPieces.Rook(x, y, color)
                    elif piece_type == 'n':
                        piece = chessPieces.Knight(x, y, color)
                    elif piece_type == 'b':
                        piece = chessPieces.Bishop(x, y, color)
                    elif piece_type == 'q':
                        piece = chessPieces.Queen(x, y, color)
                    elif piece_type == 'k':
                        piece = chessPieces.King(x, y, color)
                        if color == chessPieces.ChessPiece.CHESS_PIECE_WHITE:
    ...
    def get_legal_moves(self, piece_color):
        chess_piece_moves = []
            ...
            for y in range(Board.HEIGHT):
                ...
                if (piece != 0):
                    if (piece.piece_color == piece_color):
                        chess_piece_moves += piece.get_legal_moves(self)
    ...
    def make_move(self, move):
        ...
        #pawn promotion
        if (piece.CHESS_PIECE_TYPE == chessPieces.Pawn.CHESS_PIECE_TYPE):
            if (piece.y_pos == 0 or piece.y_pos == Board.HEIGHT-1):
                self.chesspieces[piece.x_pos][piece.y_pos] = chessPieces.Queen(piece.x_pos, piece.y_pos, piece.piece_color)

        if (piece.CHESS_PIECE_TYPE == chessPieces.King.CHESS_PIECE_TYPE):
    ...
    def is_check(self, piece_color):
        ...
        #check the color
        other_piece_color = chessPieces.ChessPiece.CHESS_PIECE_WHITE
        if piece_color == chessPieces.ChessPiece.CHESS_PIECE_WHITE:
            other_piece_color = chessPieces.ChessPiece.CHESS_PIECE_BLACK

        #find legal movbes
        for move in self.get_legal_moves(other_piece_color):
            ...
            for x_pos in range(Board.WIDTH):
                for y_pos in range(Board.HEIGHT):
                    ...
                    if piece != 0:
                        ...
                        if piece.piece_color == piece_color and piece.CHESS_PIECE_TYPE == chessPieces.King.CHESS_PIECE_TYPE:
                            king_found = True
